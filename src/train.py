...
def load_noun_map():
    path = os.path.join(DATA_DIR, "noun_map.json")
...
def load_noun_queue():
    path = os.path.join(DATA_DIR, "noun_queue.txt")
    if os.path.exists(path):
        with open(path, "r") as f:
...
def save_corpus(pairs, path=None):
    if path is None:
        path = os.path.join(DATA_DIR, "corpus.jsonl")
    with open(path, "w") as f:
        for p in pairs:
...
def parse_bank(path=None):
    if path is None:
        path = os.path.join(DATA_DIR, "bank.txt")
    groups = []
    current = None
